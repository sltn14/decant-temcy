...
<<<<<<< HEAD
      ...
      const orderItems = items.map((i) => ({
        name: `${i.product.product_name} (${i.product.brand})`,
        size: i.sizeLabel,
        price: formatRupiah(i.unitPrice * i.quantity),
      }));
      const newOrder = {
        id: orderId,
        date: new Date().toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" }),
        items: items.map((i) => `${i.product.product_name} ${i.sizeLabel}`).join(", "),
      ...
      try {
        const existing = localStorage.getItem("decant_orders");
        const orders = existing ? JSON.parse(existing) : [];
        orders.unshift(newOrder);
        localStorage.setItem("decant_orders", JSON.stringify(orders));
      } catch {
