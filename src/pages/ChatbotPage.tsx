...
export default function ChatbotPage() {
  ...
  useEffect(() => {
    const handleStorage = (e: StorageEvent) => {
      ...
      if (e.newValue === prevStorageRef.current) return;
      try {
        const parsed = JSON.parse(e.newValue ?? "[]") as ChatSession[];
        if (!Array.isArray(parsed) || parsed.length === 0) return;
        const sorted = parsed.sort(
          (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
        );
        setSessions(sorted);
        if (!sorted.find((s) => s.id === activeSessionId)) {
          setActiveSessionId(sorted[0].id);
        }
      } catch {
        // ignore parse errors
  ...
  // Also poll localStorage for same-tab updates (storage event only fires cross-tab)
  useEffect(() => {
    const interval = setInterval(() => {
      try {
        const raw = localStorage.getItem(CHAT_HISTORY_KEY);
        if (!raw || raw === prevStorageRef.current) return;
        prevStorageRef.current = raw;
        const parsed = JSON.parse(raw) as ChatSession[];
        if (!Array.isArray(parsed) || parsed.length === 0) return;
        const sorted = parsed.sort(
          (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
